...
# Welcome back! In this course we're going to be putting our web scraping skills to the test by 
# aggregating and summarizing data from many different pages.
...
# get_html('https://en.wikipedia.org/wiki/List_of_old-growth_forests', './html_docs/old_growth.html')

with open('html_docs/old_growth.html', "r", encoding='utf-8') as f:
    html = f.read()
...
# Get all tables
# Note the discrepancy with the jquery-tablesorter in the class name in the browser tools!
...
# We need keys for the tables at the highest level of the dictionary
# Label tables (Europe, Australia, etc.)
# Some of the tables have subheadings though (for instance 'Canada' and 'United States' under 'North America')
# So we'll actually need to find *either* an h3 or an h2 tag, whichever is closest!

for t in tables:
    heading = t.find_previous(['h2', 'h3']).text.replace('[edit]','')
    data[heading] = t
    print(heading)
...
# Now that we have our tables stored and labeled in a dictionary,
# let's take a look at the content of the tables. 
# First, in the browser tools.
# Each table row tag contains several table data <td> tags.
table = data["Australia"]
first_row = table.tr 
for td in first_row:
    print(td.text, '\n')
# Aha! A thing to watch out for. The column names are still part of the table as far as HTML is concerned,
# so the first row in each table is actually going to be the column names.

# It's not a bad idea to have these on hand, so we'll save them to a list.
columns = []
for td in first_row:
    if td.text.strip() != '': 
...
# But back to our original intent here.
rows = table.find_all('tr')
print(rows[1], '\n')

# Collecting data from table rows
# Let's get all the <td> of a single row.
example_row = rows[1]
...
def clean_row_data(row: dict):
    ...
    for k in row.keys(): # Iterate through table columns
        ...
        if re.match(r'\s', val.text): # We need to match all whitespace because some of these cells might have a variable number of space characters.
            row[k] = 'No data'
        ...
        if k == 'Old-growth extent' and row[k] != 'No data':
            ...
            data = re.search(r'\d+(?:,\d{3})*(?:\.\d*)? (?:hectares|square kilometres|ha|acres|sq mi)', data).group()
            # Also decimal points!
...
europe = data['Europe']
france = [x for x in europe if 'France' in x['Country'].text] # Note that it's helpful to not look for an exact match, as there are often newlines, random unicode chars, etc.
print(len(france))
...
print(km_tags)
area_tag = km_tags[0]
b_area = re.search(r'\d+(?:,\d{3})*(?:\.\d*)?', area_tag)
...
for row in bulgaria_rows:
    ...
    f_data = re.search(r'\d+(?:,\d{3})*(?:\.\d*)?', f_data).group()
...
for r in us_table:
    if 'Old-growth forest type' in r:
